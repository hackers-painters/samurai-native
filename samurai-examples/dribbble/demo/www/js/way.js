(function (root, factory) {
	...
	*/
	...
	WAY.prototype.updateForms = function() {
		...
		var self = this;
		var selector = "form[" + tagPrefix + "-path]";
		...
		for (var i in elements) {
			...
			w.dom(form).removeAttr(tagPrefix + "-path");
	...
	var setEventListeners = function() {
			...
			w.dom("[" + tagPrefix + "-path]").on("input change", eventInputChange);
			w.dom("[" + tagPrefix + "-clear]").on("click", eventClear);
			w.dom("[" + tagPrefix + "-item-delete]").on("click", eventRemove);
			w.dom("[" + tagPrefix + "-item-add]").on("click", eventPush);
