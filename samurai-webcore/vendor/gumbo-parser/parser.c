...
static bool all_attributes_match(
  ...
  for (size_t i = 0; i < attr1->length; ++i) {
...
static GumboError* parser_add_parse_error(GumboParser* parser, const GumboToken* token) {
  ...
  for (size_t i = 0; i < state->_open_elements.length; ++i) {
...
static GumboNode* create_Elementfrom_token(
...
// Convenience method that combines create_Elementfrom_token and
// insert_element, inserting the generated element directly into the current
// node.  Returns the node inserted.
static GumboNode* insert_Elementfrom_token(
    GumboParser* parser, GumboToken* token) {
  GumboNode* element =
      create_Elementfrom_token(parser, token, GUMBO_NAMESPACE_HTML);
...
// inserted.
static GumboNode* insert_Elementof_tag_type(
...
static GumboNode* insert_foreign_element(
    GumboParser* parser, GumboToken* token, GumboNamespaceEnum tag_namespace) {
  assert(token->type == GUMBO_TOKEN_START_TAG);
  GumboNode* element = create_Elementfrom_token(parser, token, tag_namespace);
  insert_element(parser, element, false);
...
static void run_generic_parsing_algorithm(
  ...
  insert_Elementfrom_token(parser, token);
...
// document.
static bool has_an_Elementin_specific_scope(
...
// GumboVector.
#define DECLARE_ONE_ElementGUMBO_VECTOR(varname, from_var) \
...
static bool has_an_Elementin_scope(GumboParser* parser, GumboTag tag) {
  DECLARE_ONE_ElementGUMBO_VECTOR(tags, tag);
  return has_an_Elementin_specific_scope(
...
// much of the algorithm from has_an_Elementin_specific_scope because the
...
// Like has_an_Elementin_scope, but restricts the expected tag to a range of
// possible tag names instead of just a single one.
static bool has_an_Elementin_scope_with_tagname(GumboParser* parser, ...) {
  GumboVector tags;
  ...
  bool found = has_an_Elementin_specific_scope(
      parser, &tags, false, GUMBO_TAG_APPLET, GUMBO_TAG_CAPTION, GUMBO_TAG_HTML,
      GUMBO_TAG_TABLE, GUMBO_TAG_TD, GUMBO_TAG_TH, GUMBO_TAG_MARQUEE,
      GUMBO_TAG_OBJECT, GUMBO_TAG_MI, GUMBO_TAG_MO, GUMBO_TAG_MN, GUMBO_TAG_MS,
      GUMBO_TAG_MTEXT, GUMBO_TAG_ANNOTATION_XML, GUMBO_TAG_FOREIGNOBJECT,
...
static bool has_an_Elementin_list_scope(GumboParser* parser, GumboTag tag) {
  DECLARE_ONE_ElementGUMBO_VECTOR(tags, tag);
  return has_an_Elementin_specific_scope(
      parser, &tags, false, GUMBO_TAG_APPLET, GUMBO_TAG_CAPTION, GUMBO_TAG_HTML,
      GUMBO_TAG_TABLE, GUMBO_TAG_TD, GUMBO_TAG_TH, GUMBO_TAG_MARQUEE,
...
static bool has_an_Elementin_button_scope(GumboParser* parser, GumboTag tag) {
  DECLARE_ONE_ElementGUMBO_VECTOR(tags, tag);
  return has_an_Elementin_specific_scope(
      parser, &tags, false, GUMBO_TAG_APPLET, GUMBO_TAG_CAPTION, GUMBO_TAG_HTML,
      GUMBO_TAG_TABLE, GUMBO_TAG_TD, GUMBO_TAG_TH, GUMBO_TAG_MARQUEE,
...
static bool has_an_Elementin_table_scope(GumboParser* parser, GumboTag tag) {
  DECLARE_ONE_ElementGUMBO_VECTOR(tags, tag);
  return has_an_Elementin_specific_scope(
...
static bool has_an_Elementin_select_scope(GumboParser* parser, GumboTag tag) {
  DECLARE_ONE_ElementGUMBO_VECTOR(tags, tag);
  return has_an_Elementin_specific_scope(
...
static bool close_table(GumboParser* parser) {
  if (!has_an_Elementin_table_scope(parser, GUMBO_TAG_TABLE)) {
    return false;
  }
...
static bool close_current_cell(GumboParser* parser, const GumboToken* token) {
  if (has_an_Elementin_table_scope(parser, GUMBO_TAG_TD)) {
    assert(!has_an_Elementin_table_scope(parser, GUMBO_TAG_TH));
...
static bool maybe_implicitly_close_p_tag(GumboParser* parser, GumboToken* token) {
  if (has_an_Elementin_button_scope(parser, GUMBO_TAG_P)) {
...
static bool adoption_agency_algorithm(
  ...
  for (int i = 0; i < 8; ++i) {
    ...
    if (!has_an_Elementin_scope(parser, formatting_node->v.element.tag)) {
      parser_add_parse_error(parser, token);
...
static bool handle_before_html(GumboParser* parser, GumboToken* token) {
  if (token->type == GUMBO_TOKEN_DOCTYPE) {
    ...
    GumboNode* html_node = insert_Elementfrom_token(parser, token);
    ...
    GumboNode* html_node = insert_Elementof_tag_type(
...
static bool handle_before_head(GumboParser* parser, GumboToken* token) {
  if (token->type == GUMBO_TOKEN_DOCTYPE) {
    ...
    GumboNode* node = insert_Elementfrom_token(parser, token);
    ...
    GumboNode* node = insert_Elementof_tag_type(
...
static bool handle_after_head(GumboParser* parser, GumboToken* token) {
  ...
  if (token->type == GUMBO_TOKEN_WHITESPACE) {
    ...
    insert_Elementof_tag_type(parser, GUMBO_TAG_BODY, GUMBO_INSERTION_IMPLIED);
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_BODY);
...
static bool handle_in_body(GumboParser* parser, GumboToken* token) {
  ...
  if (token->type == GUMBO_TOKEN_NULL) {
    ...
    return true;
                    ...
                    GUMBO_TAG_LAST)) {
    if (!has_an_Elementin_scope(parser, GUMBO_TAG_BODY)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
      return false;
    }
    ...
    if (has_an_Elementin_scope(parser, GUMBO_TAG_BUTTON)) {
      parser_add_parse_error(parser, token);
    ...
    if (!has_an_Elementin_scope(parser, tag)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
      return false;
    }
    ...
    if (!has_an_Elementin_list_scope(parser, GUMBO_TAG_LI)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
      return false;
    }
    ...
    if (!has_an_Elementin_scope(parser, token_tag)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
      return false;
    }
    return implicitly_close_tags(parser, token, token_tag);
                    ...
                    GUMBO_TAG_H4, GUMBO_TAG_H5, GUMBO_TAG_H6, GUMBO_TAG_LAST)) {
    if (!has_an_Elementin_scope_with_tagname(
    ...
    reconstruct_active_formatting_elements(parser);
    add_formatting_element(parser, insert_Elementfrom_token(parser, token));
    return success;
    ...
    bool result = true;
    reconstruct_active_formatting_elements(parser);
    if (has_an_Elementin_scope(parser, GUMBO_TAG_NOBR)) {
      result = false;
      parser_add_parse_error(parser, token);
    ...
    return true;
                    ...
                    GUMBO_TAG_OBJECT, GUMBO_TAG_LAST)) {
    GumboTag token_tag = token->v.end_tag;
    if (!has_an_Elementin_table_scope(parser, token_tag)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
      return false;
    }
    ...
    GumboNode* input = insert_Elementof_tag_type(
    ...
    return true;
                    ...
                    GUMBO_TAG_LAST)) {
    bool success = true;
    if (has_an_Elementin_scope(parser, GUMBO_TAG_RUBY)) {
      generate_implied_end_tags(parser, GUMBO_TAG_LAST);
    }
...
static bool handle_in_table(GumboParser* parser, GumboToken* token) {
  ...
  if (token->type == GUMBO_TOKEN_CHARACTER ||
    ...
    state->_form_element = insert_Elementfrom_token(parser, token);
    pop_current_node(parser);
    return false;
  } else if (token->type == GUMBO_TOKEN_EOF) {
...
static bool handle_in_caption(GumboParser* parser, GumboToken* token) {
  if (tag_in(token, kStartTag, GUMBO_TAG_CAPTION, GUMBO_TAG_COL,
             ...
             tag_in(token, kEndTag, GUMBO_TAG_CAPTION, GUMBO_TAG_TABLE,
                    GUMBO_TAG_LAST)) {
    if (!has_an_Elementin_table_scope(parser, GUMBO_TAG_CAPTION)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
      return false;
    }
...
static bool handle_in_table_body(GumboParser* parser, GumboToken* token) {
  if (tag_is(token, kStartTag, GUMBO_TAG_TR)) {
    ...
    parser_add_parse_error(parser, token);
    clear_stack_to_table_body_context(parser);
    insert_Elementof_tag_type(parser, GUMBO_TAG_TR, GUMBO_INSERTION_IMPLIED);
    parser->_parser_state->_reprocess_current_token = true;
    set_insertion_mode(parser, GUMBO_INSERTION_MODE_IN_ROW);
    return false;
  } else if (tag_in(token, kEndTag, GUMBO_TAG_TBODY, GUMBO_TAG_TFOOT,
                    GUMBO_TAG_THEAD, GUMBO_TAG_LAST)) {
    if (!has_an_Elementin_table_scope(parser, token->v.end_tag)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
      return false;
    }
    ...
    if (!(has_an_Elementin_table_scope(parser, GUMBO_TAG_TBODY) ||
          has_an_Elementin_table_scope(parser, GUMBO_TAG_THEAD) ||
          has_an_Elementin_table_scope(parser, GUMBO_TAG_TFOOT))) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
      return false;
    }
...
static bool handle_in_row(GumboParser* parser, GumboToken* token) {
  if (tag_in(token, kStartTag, GUMBO_TAG_TH, GUMBO_TAG_TD, GUMBO_TAG_LAST)) {
    ...
    if (!has_an_Elementin_table_scope(parser, desired_tag)) {
...
static bool handle_in_cell(GumboParser* parser, GumboToken* token) {
  if (tag_in(token, kEndTag, GUMBO_TAG_TD, GUMBO_TAG_TH, GUMBO_TAG_LAST)) {
    ...
    if (!has_an_Elementin_table_scope(parser, GUMBO_TAG_TH) &&
        !has_an_Elementin_table_scope(parser, GUMBO_TAG_TD)) {
...
static bool handle_in_select(GumboParser* parser, GumboToken* token) {
  if (token->type == GUMBO_TOKEN_NULL) {
    ...
    if (!has_an_Elementin_select_scope(parser, GUMBO_TAG_SELECT)) {
      parser_add_parse_error(parser, token);
      ignore_token(parser);
      return false;
    }
